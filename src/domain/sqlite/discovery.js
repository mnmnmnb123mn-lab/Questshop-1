...
export function announceCustomerDiscovery(db, { notificationId, actorId }) {
  const timestamp = nowMs();
  return withImmediateTransaction(db, () => {
    const notification = caseNotification(db, notificationId);
    ...
    db.prepare(`UPDATE quests SET announcement_status='QUEUED',state_version=state_version+1,updated_at=? WHERE quest_id=? AND state_version=?`)
      .run(timestamp, quest.quest_id, quest.state_version);
    const auditId = randomUUID();
    db.prepare(`INSERT INTO admin_audit(id,actor_id,action,target_type,target_id,reason,after_json,trace_id,created_at)
