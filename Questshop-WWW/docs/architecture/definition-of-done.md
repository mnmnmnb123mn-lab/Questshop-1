...
| Discord UI | persistent setup/update/move; Ephemeral secrets; allowed-mention allowlist; terminal controls disabled; Thai/mobile-safe content; `QUEST_AUTO` keeps one durable anchor, fixed Owner-approved title/copy, bundled animated GIF thumbnail and exact `quest-auto-demo.gif` rendered inside the embed, `QUEST_HISTORY` keeps profile thumbnail, linked Quest progress and one bundled banner image, no visible technical footer, dynamic configured price/range, stale-price/media reconciliation and no active duplicate panel |
...
- `renderQuestAuto()` renders **Discord Quest Auto**, the approved Discord Orbs / Discord Token copy,
  `attachment://quest-auto-thumbnail.gif` as the animated thumbnail and `attachment://quest-auto-demo.gif` as the embed image.
...
- `src/discord/assets/quest-auto-thumbnail.gif` is the approved animated thumbnail: `822,513` bytes and SHA-256
  `2d1e0e2c09138ac53384ac6272f4c8a9eedff28e2fe227ee06e26f7ef37a6542`.
- Missing/legacy attachments are replaced on the existing surface, while an already-correct pair is preserved to avoid
  duplicate uploads.
